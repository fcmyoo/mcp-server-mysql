...
- `ENABLE_PII_REDACTION`: When set to `"true"`, read-only query results are walked and likely PII values are replaced with shape-preserving partial masks before being returned (default: `"false"`). Detection combines a built-in column-name list (`email`, `ssn`, `phone`, `first_name`, `address`, `credit_card`, `password`, `api_key`, `token`, etc.) with regex scanning of values (email, US phone, SSN, IPv4, Luhn-valid credit card). Examples:
  ...
  **Hardening guards (defence in depth).** When `ENABLE_PII_REDACTION=true` is set, three additional pre-execution guards are also enabled by default. They are independent of the value-level redactor and protect against cases the redactor cannot, on its own, catch (column aliases, schema enumeration, accidental wildcards). Each guard has a dedicated opt-out for tables / workflows that don't need it:
  ...
  - `PII_ALLOW_INTROSPECTION` (default `"false"`): when `false`, schema-introspection statements are rejected so the LLM cannot enumerate PII column names directly via SQL. Covers `SHOW COLUMNS`, `SHOW FULL COLUMNS`, `SHOW FIELDS`, `SHOW CREATE TABLE`, `SHOW CREATE VIEW`, `SHOW INDEX(ES)`, `SHOW KEYS`, `SHOW TABLES`, `SHOW TABLE STATUS`, `DESCRIBE`, `DESC`, `EXPLAIN <table>`, and any `SELECT` that touches `information_schema` or `mysql` (including via `JOIN` or subquery). The MCP resource endpoint (`mysql://tables/{name}`) already filters PII columns out of its response, so the LLM should use that path instead. Set to `"true"` to allow direct introspection.
  ...
  **Known out-of-scope cases** (things the redactor does *not* catch — do not rely on it for these):
  ...
  - **Schema and resource listings (when the introspection guard is disabled).** With `PII_ALLOW_INTROSPECTION=true` (or with `ENABLE_PII_REDACTION=false`), table names, column names, and `information_schema` responses are returned as-is even when they contain words like `email` or `ssn`. The introspection guard (default-on under `ENABLE_PII_REDACTION=true`) rejects these queries entirely; the filtered MCP resource endpoint (`mysql://tables/{name}`) is the supported path for schema discovery.
